...
# Sum of all 7 controls' risk reduction — used as the denominator for reduction_pct
# instead of a hardcoded magic number.
_CATALOGUE_TOTAL_REDUCTION_INR = sum(c["risk_reduction_inr"] for c in CONTROLS)
...
def optimize_budget(budget_inr: float) -> dict:
    ...
    try:
        from pulp import LpProblem, LpVariable, LpMaximize, lpSum, COIN_CMD, value
    ...
    reduction_pct = (
        round(total_reduction / _CATALOGUE_TOTAL_REDUCTION_INR * 100, 1)
        if _CATALOGUE_TOTAL_REDUCTION_INR else 0
    )
    return {
